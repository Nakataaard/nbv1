//======Made By Pebri [Recode by: Nakata]======\\
...
const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
          if(time2 < "23:59:00"){
          var ucapanWaktu = 'SelamatMalam🌕, Kok Belum Tidur?'
}
          if(time2 < "19:00:00"){
          var ucapanWaktu = 'SelamatMalam🌔'
}
          if(time2 < "18:00:00"){
          var ucapanWaktu = 'SelamarMalam🌇'
}
          if(time2 < "15:00:00"){
          var ucapanWaktu = 'SelamatSore🌄'

}         if(time2 < "11:00:00"){
          var ucapanWaktu = 'SelamatSiang🌤️'
}
          if(time2 < "10:00:00"){
          var ucapanWaktu = 'SelamatPagi🌅'
}
          if(time2 < "05:00:00"){
          var ucapanWaktu = 'SelamatPagi🌕'
}
	...
	zee.on('group-participants-update', async (anu) => {
	  const isWelkom = _welkom.includes(anu.jid)
		...
		try {
            ...
            zee.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Full Fitur, Tolong Jadikan Admin Ya, Ketik .menu', 'conversation')
            }
            ...
            if (!isWelkom) return
             if (anu.action == 'add' && !mem.includes(zee.user.jid)) {     
              ...
              teks = `Selamat Datang Kak *@${mem.split('@')[0]}*\n*Welcome in group *${groupName}*\n*Nomor :* ${mem.replace('@s.whatsapp.net', '')}\nSilahkan Baca Deskripsi Groupnya Kak\n\n*Semoga betah~~*`
              buff = await getBuffer(pic)
              zee.sendMessage(mdata.id, { contentText: `${teks}`, footerText: `Welcome Message By *Nakata*`, buttons: [{buttonId: `.selamatdatang`,buttonText:{displayText: 'irasshaimas'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
		}
		...
		if (!isWelkom) return
            if (anu.action == 'remove' && !mem.includes(zee.user.jid)) {
                ...
                memeg = mdata.participants.length
                out = `Selamat Tinggal Kak @${num.split('@')[0]} Semoga Jasatmu Tidak Tenang!`
               buff = await getBuffer(pic)
               zee.sendMessage(mdata.id, { contentText: `${out}`, footerText: `Left Message By *Nakata*`, buttons: [{buttonId: `.bay`,buttonText:{displayText: 'SAYONARA👋'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
            }
			} catch (e) {
